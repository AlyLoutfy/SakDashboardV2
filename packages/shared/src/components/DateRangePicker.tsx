...
export default function DateRangePicker({ value, onChange, className, placeholder = "Select dates" }: DateRangePickerProps) {
  ...
  return (
    <div className="relative" ref={popoverRef}>
      {/* Trigger */}
      <div onClick={() => setIsOpen(!isOpen)} className={`relative cursor-pointer group ${className}`}>
      ...
      {/* Popover Content */}
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 p-0 bg-white border border-gray-100 rounded-2xl shadow-lg overflow-hidden z-[100] w-auto">
          <div className="p-4 space-y-3">
            ...
            <DayPicker
              ...
              classNames={{
                ...
                head_cell: "text-center text-[10px] font-semibold text-gray-400 flex-1 py-1",
                row: "flex gap-0.5 w-full",
                cell: "text-center text-sm p-0.5 relative flex-1",
                day: "h-8 w-8 mx-auto rounded-full text-xs font-normal transition-all hover:bg-gray-100 text-gray-700",
                day_range_start: "day-range-start",
                day_range_end: "day-range-end",
                day_selected: "day-selected",
                day_today: "bg-transparent text-indigo-600 font-bold border border-indigo-200",
