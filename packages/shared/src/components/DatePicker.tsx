...
export default function DatePicker({ value, onChange, className, placeholder = "Select date" }: DatePickerProps) {
  ...
  return (
    <div className="relative" ref={popoverRef}>
      {/* Trigger */}
      <div onClick={() => setIsOpen(!isOpen)} className={`relative cursor-pointer ${className}`}>
      ...
      {/* Popover Content */}
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 p-0 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden z-50">
          <style>{`
            .rdp {
              --rdp-cell-size: 32px;
