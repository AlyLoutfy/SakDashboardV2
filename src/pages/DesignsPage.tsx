...
const Design1 = ({ connected }: { connected: boolean }) => (
  <Card className="p-4 border border-gray-200 dark:border-zinc-800 shadow-sm hover:shadow-md transition-shadow">
    <div className="flex justify-between">
       <div className="p-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 rounded-lg h-fit">
          {MOCK_DATA.icon}
       </div>
       {connected && <Chip color="success" variant="soft" size="sm">Active</Chip>}
    </div>
    <div className="mt-4">
...
const Design2 = ({ connected }: { connected: boolean }) => (
  <Card className="p-4 border border-gray-100 dark:border-zinc-800 shadow-none bg-gray-50 dark:bg-zinc-900/50">
    <div className="flex items-center gap-4">
       <div className="p-3 bg-white dark:bg-zinc-800 rounded-full shadow-sm text-blue-600">
          {MOCK_DATA.icon}
       </div>
       ...
       <div className="flex flex-col gap-1 items-end">
          {connected ? (
             <Button size="sm" isIconOnly variant="ghost"><Settings2 size={16}/></Button>
          ) : (
             <Button size="sm" variant="secondary" className="h-8 text-xs">Connect</Button>
          )}
       </div>
    </div>
  </Card>
  ...
  <Card className="p-6 flex flex-col items-center text-center border-none shadow-lg shadow-gray-100/50 dark:shadow-none bg-white dark:bg-zinc-900">
    ...
    {connected ? (
       <div className="flex gap-2 w-full">
         <Button fullWidth variant="danger-soft">Disconnect</Button>
         <Button fullWidth variant="secondary">Settings</Button>
       </div>
    ) : (
       <Button fullWidth variant="primary" className="shadow-blue-500/30">Connect Now</Button>
    )}
  </Card>
);
  ...
  <Card className={`p-0 overflow-hidden border-none ${connected ? 'bg-gradient-to-br from-indigo-900 to-blue-900 text-white' : 'bg-white dark:bg-zinc-900 border border-gray-200'}`}>
    <div className="p-6 relative z-10">
       <div className="flex justify-between items-start">
          <div className={`p-2 rounded-lg ${connected ? 'bg-white/20' : 'bg-gray-100 dark:bg-zinc-800 text-gray-600'}`}>
            {MOCK_DATA.icon}
          </div>
  ...
  <Card className="p-3 border border-gray-200 dark:border-zinc-800 flex items-center gap-4 hover:border-gray-300 transition-colors">
     ...
     {connected ? (
        <div className="flex items-center gap-2">
           ...
           <Button isIconOnly size="sm" variant="ghost"><MoreHorizontal size={16} /></Button>
        </div>
     ) : (
        <Button size="sm" variant="secondary" className="h-8">Connect</Button>
     )}
  </Card>
);
...
const Design7 = ({ connected }: { connected: boolean }) => (
  <Card className="overflow-hidden border-none shadow-md">
     ...
     <div className="pt-10 pb-6 px-6 text-center">
        ...
        <div className="mt-6">
           {connected ? (
              <Button className="w-full" variant="secondary">Active</Button>
           ) : (
              <Button className="w-full" variant="primary">Install</Button>
           )}
        </div>
     </div>
  </Card>
...
const Design9 = ({ connected }: { connected: boolean }) => (
  <Card className="p-5 bg-[#f0f4f8] dark:bg-zinc-900 border-none shadow-[5px_5px_10px_#d1d9e6,-5px_-5px_10px_#ffffff] dark:shadow-none">
     <div className="flex justify-between items-start"> 
        <div className="bg-[#f0f4f8] dark:bg-zinc-800 p-3 rounded-xl shadow-[inset_3px_3px_6px_#d1d9e6,inset_-3px_-3px_6px_#ffffff] dark:shadow-none">
           {MOCK_DATA.icon}
        </div>
     </div>
     <div className="mt-4">
     ...
     <div className="mt-6">
        <Button 
          className="w-full shadow-[3px_3px_6px_#d1d9e6,-3px_-3px_6px_#ffffff] dark:shadow-none bg-[#f0f4f8] dark:bg-zinc-800 text-blue-600 font-semibold" 
          variant="ghost"
        >
     ...
     <div className="p-5">
        ...
        <div className="mt-5 pt-3 border-t border-dashed border-gray-200 dark:border-zinc-800">
           {connected ? (
              ...
              <Button size="sm" fullWidth variant="secondary">Activate</Button>
           )}
        </div>
     </div>
  </Card>
    ...
    <div className="space-y-12 pb-20" ref={topRef}>
      <div className="max-w-3xl">
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-2 text-lg">
          Exploration of designs for various UI components.
        </p>
      </div>
        ...
        <div className="mb-8 border-b border-gray-200 dark:border-zinc-800">
            <div className="flex gap-6 relative">
                <button 
                    ...
                    className={`px-4 py-3 font-medium cursor-pointer outline-none transition-colors relative ${activeTab === 'cards' ? 'text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    Integration Cards
                ...
                <button 
                    ...
                    className={`px-4 py-3 font-medium cursor-pointer outline-none transition-colors relative ${activeTab === 'mapping' ? 'text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    Field Mapping
            ...
            <div key={`cards-${Date.now()}`} className="grid grid-cols-1 md:grid-cols-2 gap-12 pt-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                {DESIGNS.map(({ id, name, Component }) => (
                    <div key={id} className="space-y-4">
                        <h2 className="text-xl font-semibold text-gray-400">Design {id}: {name}</h2>
