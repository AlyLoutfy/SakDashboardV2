  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="max-w-3xl">
        <h1 className="text-3xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-400">Integrations</h1>
        <p className="text-gray-500 dark:text-gray-400 mt-2 text-lg">Connect your favorite tools to streamline your lead generation workflow.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {AVAILABLE_INTEGRATIONS.map((tool) => {
          ...
          return <IntegrationCard key={tool.id} id={tool.id} name={tool.name} description={tool.description} icon={tool.icon} data={data} onConnect={() => handleConnect(tool.id)} onManage={() => handleManage(tool.id)} isPending={isConnecting && activeIntegrationId === tool.id} />;
        })}
      </div>
    </div>
  );
