...
import { formatCurrency } from "../../store/chequesStore";

export type CashFlowMode = "due_date" | "collection_date";
...
const currentMonthLabel = new Intl.DateTimeFormat("en-US", { month: "short", year: "numeric" }).format(
  new Date(currentMonthKey + "-01")
);

const CashFlowChart = ({ mode, fromMonth, toMonth, allData }: Props) => {
  ...
  const getCollected = (d: CashFlowDataPoint) =>
  ...
  const gridLines = [0.25, 0.5, 0.75, 1].map((pct) => ({ value: niceMax * pct, pct: pct * 100 }));
                ...
                <tr className="bg-gray-50 border-t border-gray-200">
                  ...
                  <td className="px-3 py-2 text-right tabular-nums font-bold">
                    <span className={visibleTotalExpected - visibleTotalCollected > 0 ? "text-amber-600" : "text-gray-300"}>
                      {visibleTotalExpected - visibleTotalCollected > 0
                        ? formatCurrency(visibleTotalExpected - visibleTotalCollected)
                        : "—"}
                    </span>
                  </td>
                  <td className="px-3 py-2">
                    <div className="flex items-center gap-1.5 justify-end">
                      ...
                      <span className="text-[10px] font-bold text-gray-700 tabular-nums w-7 text-right">
